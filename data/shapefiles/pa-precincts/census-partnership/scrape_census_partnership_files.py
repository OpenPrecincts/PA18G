...
partnership_url = 'https://www.census.gov/geo/partnerships/pvs/partnership19v1/st42_pa.html'
...
p = Path.home() / "pgp" / "pa-2018" / "data" / "census-partnership-files"
...
for zip in p.glob('*.zip'):
    unpack_archive(zip, extract_dir= up) # first unzip - each zip has two zipped files in it!

for zip in up.glob('*.zip'):
    unpack_archive(zip, extract_dir= ex) # second unzip
...
assert 67 == n_VTD_files # if error, go back and find missed county download 
